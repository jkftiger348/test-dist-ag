...
In this document we primarily discuss running a MMR (repl) replication cluster
...
The container created by this Dockerfile is pushed to the Docker Hub using an account you specify.  We'll use the pseudo account name *dockeraccount* in this document to stand in for whatever docker account you specify.
...
This Dockerfile uses  dockeraccount/ag:latest and when we process
the Dockerfile we give it the tag dockeraccount/agrepl:latest
...
a sophisiticated platform to run (such as GCP or AWS).
...
work. You'll want to sure you can push to the hub without needing
...
### Declare docker account
if your Docker Hub account is `mydocker` then
```
% make name=mydocker setup
```
### install AllegroGraph
Change to the ag account and build a container with AllegroGraph installed.
...
% cd ..
```
### Create cluster-aware AllegroGraph container
Add scripts to create a container that will either
create a cluster or join a cluster when started.
```
% cd agrepl
% make build
% make push
...
You may already have one running or you may need to create
one.  Both GCP and AWS have ways of creating a cluster using
a web UI or a shell command.
When you've got your cluster setup you can do
```
% kubectl get nodes
```
and you'll see your nodes listed.
At that point you can move into the next step.
...
% make doall
```
You'll see when it displays the services that there isn't an external IP address allocated for the load balancers   It can take a few minutes for an external IP address to be allocated and the load balancer setup so keep
```
% kubectl  get svc
```
until you see an IP address given, and even then it may not work for a minute or two after that.
...
```
Login as user `test` with the password `xyzzy`.
You'll see a repository `myrepl` listed.  Click on that.
Mid way down you'll see a link titled
```
...
### Namespaces
All objects created in Kubernetes have a name which is chosen by the programmer or Kubernetes based on a name given by the programmer.  Most names have an
associated namespace.  The combination of namespace and name must be unique among all objects.   The reason for having a namespace is that it prevents name clashes between multiple projects running in the same cluster who might both choose to name an object `foo`.

The default namespace is named `default`.

One big advantage using namespaces is that if you delete a namespace you delete all objects whose name is in that namespace.  This is useful because a project i Kubernetes uses a lot of different types of objects and if you want to delete all the objects you've added to  kubernetes cluster it can take a while to find all the objects by type and then delete them.  However if you put all the objects in one namespace then you need only delete the namespace and you're done.
...
Once you've gotten the ag and agrepl containers built and pushed to the Docker Hub you need only link a set of machines running docker together into
